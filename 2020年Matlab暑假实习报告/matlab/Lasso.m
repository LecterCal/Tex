...
%cvx
cvx_begin quiet
    cvx_precision low %cvx_precision--参考cvx document
    variable x(n)
    minimize(0.5*sum_square(A*x - b) + gamma*norm(x,1))
cvx_end
% Structure of convex problem
h.x_cvx = x;
h.p_cvx = cvx_optval;
h.cvx_toc = toc;
%Proximal gradient
...
h.admm_toc = toc;
fprintf('CVX 消耗时间: %.2f seconds.\n', h.cvx_toc);
fprintf('Proximal gradient 消耗时间: %.2f seconds.\n', h.prox_grad_toc);
fprintf('Fast prox gradient 消耗时间: %.2f seconds.\n', h.fast_toc);
fprintf('ADMM 消耗时间: %.2f seconds.\n', h.admm_toc);
...
fig = figure;
plot(1:K, h.admm_optval, 'b-');legend('ADMM');
plot(1:K, h.prox_optval, 'r-');legend('Proximal gradient');
plot(1:K, h.fast_optval, 'g-');legend('Accelerated');
